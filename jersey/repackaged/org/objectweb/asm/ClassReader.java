    ...
    private void readCode(final MethodVisitor mv, final Context context, int u) {
        ...
        while (u < codeEnd) {
            ...
            while (tanns != null && tann < tanns.length && ntoff <= offset2) {
                if (ntoff == offset2) {
                    final int v = this.readAnnotationTarget(context, tanns[tann]);
            ...
            while (itanns != null && itann < itanns.length && nitoff <= offset2) {
                if (nitoff == offset2) {
                    final int v = this.readAnnotationTarget(context, itanns[itann]);
        ...
        if (tanns != null) {
            for (int k = 0; k < tanns.length; ++k) {
                if (this.readByte(tanns[k]) >> 1 == 32) {
                    int v2 = this.readAnnotationTarget(context, tanns[k]);
                    v2 = this.readAnnotationValues(v2 + 2, c, true, mv.visitLocalVariableAnnotation(context.typeRef, context.typePath, context.start, context.end, context.index, this.readUTF8(v2, c), true));
                }
            }
        }
        if (itanns != null) {
            for (int k = 0; k < itanns.length; ++k) {
                if (this.readByte(itanns[k]) >> 1 == 32) {
                    int v2 = this.readAnnotationTarget(context, itanns[k]);
                    v2 = this.readAnnotationValues(v2 + 2, c, true, mv.visitLocalVariableAnnotation(context.typeRef, context.typePath, context.start, context.end, context.index, this.readUTF8(v2, c), false));
                }
            }
        }
        while (attributes != null) {
