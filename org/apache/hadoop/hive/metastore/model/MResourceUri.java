...
public class MResourceUri implements Detachable, PersistenceCapable
{
    ...
    public MResourceUri(final int resourceType, final String uri) {
        this.resourceType = resourceType;
        this.uri = uri;
    }
    
    public int getResourceType() {
        return jdoGetresourceType(this);
    }
    
    public void setResourceType(final int resourceType) {
        jdoSetresourceType(this, resourceType);
    }
    ...
    static {
        ...
        JDOImplHelper.registerClass(___jdo$loadClass("org.apache.hadoop.hive.metastore.model.MResourceUri"), MResourceUri.jdoFieldNames, MResourceUri.jdoFieldTypes, MResourceUri.jdoFieldFlags, MResourceUri.jdoPersistenceCapableSuperclass, new MResourceUri());
    }
    ...
    public void jdoMakeDirty(final String fieldName) {
        ...
        if (this.jdoIsDetached() && fieldName != null) {
            ...
            int i = 0;
            while (i < MResourceUri.jdoFieldNames.length) {
                if (MResourceUri.jdoFieldNames[i].equals(fldName)) {
                    if (((BitSet)this.jdoDetachedState[2]).get(i + MResourceUri.jdoInheritedFieldCount)) {
                        ((BitSet)this.jdoDetachedState[3]).set(i + MResourceUri.jdoInheritedFieldCount);
                        return;
                    }
    ...
    public PersistenceCapable jdoNewInstance(final StateManager sm) {
        final MResourceUri result = new MResourceUri();
        result.jdoFlags = 1;
    ...
    public void jdoReplaceField(final int index) {
        ...
        switch (index) {
            case 0: {
                this.resourceType = this.jdoStateManager.replacingIntField(this, index);
                break;
            }
            case 1: {
                this.uri = this.jdoStateManager.replacingStringField(this, index);
                break;
            }
            default: {
    ...
    public void jdoProvideField(final int index) {
        ...
        switch (index) {
            case 0: {
                this.jdoStateManager.providedIntField(this, index, this.resourceType);
                break;
            }
            case 1: {
                this.jdoStateManager.providedStringField(this, index, this.uri);
                break;
            }
            default: {
                throw new IllegalArgumentException(new StringBuffer("out of field index :").append(index).toString());
    ...
    protected final void jdoCopyField(final MResourceUri obj, final int index) {
        switch (index) {
            case 0: {
    ...
    private Object jdoSuperClone() throws CloneNotSupportedException {
        final MResourceUri o = (MResourceUri)super.clone();
        o.jdoFlags = 0;
        o.jdoStateManager = null;
        return o;
    }
    
    private static int jdoGetresourceType(final MResourceUri objPC) {
        if (objPC.jdoFlags > 0 && objPC.jdoStateManager != null && !objPC.jdoStateManager.isLoaded(objPC, 0)) {
            return objPC.jdoStateManager.getIntField(objPC, 0, objPC.resourceType);
        }
    ...
    private static void jdoSetresourceType(final MResourceUri objPC, final int val) {
        if (objPC.jdoFlags != 0 && objPC.jdoStateManager != null) {
            objPC.jdoStateManager.setIntField(objPC, 0, objPC.resourceType, val);
        }
        else {
    ...
    private static String jdoGeturi(final MResourceUri objPC) {
        if (objPC.jdoFlags > 0 && objPC.jdoStateManager != null && !objPC.jdoStateManager.isLoaded(objPC, 1)) {
            return objPC.jdoStateManager.getStringField(objPC, 1, objPC.uri);
        }
    ...
    private static void jdoSeturi(final MResourceUri objPC, final String val) {
        if (objPC.jdoFlags != 0 && objPC.jdoStateManager != null) {
            objPC.jdoStateManager.setStringField(objPC, 1, objPC.uri, val);
        }
        else {
            objPC.uri = val;
            if (objPC.jdoIsDetached()) {
