...
public final class ExtrasPatternParser
{
    ...
    public static Map getPatternLayoutRules() {
        return ExtrasPatternParser.PATTERN_LAYOUT_RULES;
    }
    
    public static Map getFileNamePatternRules() {
        return ExtrasPatternParser.FILENAME_PATTERN_RULES;
    }
        ...
        while (i < patternLength) {
            ...
            switch (state) {
                ...
                case 1: {
                    ...
                    switch (c) {
                        case '-': {
                            formattingInfo = new ExtrasFormattingInfo(true, formattingInfo.isRightTruncated(), formattingInfo.getMinLength(), formattingInfo.getMaxLength());
                            continue;
                        }
                        case '!': {
                            formattingInfo = new ExtrasFormattingInfo(formattingInfo.isLeftAligned(), true, formattingInfo.getMinLength(), formattingInfo.getMaxLength());
                            continue;
                        }
                        case '.': {
                            state = 3;
                        ...
                        default: {
                            if (c >= '0' && c <= '9') {
                                formattingInfo = new ExtrasFormattingInfo(formattingInfo.isLeftAligned(), formattingInfo.isRightTruncated(), c - '0', formattingInfo.getMaxLength());
                                state = 4;
                                continue;
                            }
                ...
                case 4: {
                    currentLiteral.append(c);
                    if (c >= '0' && c <= '9') {
                        formattingInfo = new ExtrasFormattingInfo(formattingInfo.isLeftAligned(), formattingInfo.isRightTruncated(), formattingInfo.getMinLength() * 10 + (c - '0'), formattingInfo.getMaxLength());
                        continue;
                    }
                    if (c == '.') {
                        state = 3;
                ...
                case 3: {
                    currentLiteral.append(c);
                    if (c >= '0' && c <= '9') {
                        formattingInfo = new ExtrasFormattingInfo(formattingInfo.isLeftAligned(), formattingInfo.isRightTruncated(), formattingInfo.getMinLength(), c - '0');
                        state = 5;
                        continue;
                    }
                ...
                case 5: {
                    currentLiteral.append(c);
                    if (c >= '0' && c <= '9') {
                        formattingInfo = new ExtrasFormattingInfo(formattingInfo.isLeftAligned(), formattingInfo.isRightTruncated(), formattingInfo.getMinLength(), formattingInfo.getMaxLength() * 10 + (c - '0'));
                        continue;
                    }
                    i = finalizeConverter(c, pattern, i, currentLiteral, formattingInfo, converterRegistry, rules, patternConverters, formattingInfos);
                    state = 0;
        ...
        Label_0208: {
            if (!(converterObj instanceof Class)) {
                if (converterObj instanceof String) {
                    try {
                        final Class converterClass = Loader.loadClass((String)converterObj);
                        break Label_0208;
                    }
            ...
            final Class converterClass = (Class)converterObj;
            try {
    ...
    private static int finalizeConverter(final char c, final String pattern, int i, final StringBuffer currentLiteral, final ExtrasFormattingInfo formattingInfo, final Map converterRegistry, final Map rules, final List patternConverters, final List formattingInfos) {
