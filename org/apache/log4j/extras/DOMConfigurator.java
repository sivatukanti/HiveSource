...
public class DOMConfigurator implements Configurator
{
    ...
    private LoggerFactory catFactory;
    ...
    protected void setParameter(final Element elem, final PropertySetter propSetter) {
        setParameter(elem, propSetter, this.props);
    }
    ...
    public static OptionHandler parseElement(final Element element, final Properties props, final Class expectedClass) throws Exception {
        ...
        if (instance instanceof OptionHandler) {
            final OptionHandler optionHandler = (OptionHandler)instance;
            final PropertySetter propSetter = new PropertySetter(optionHandler);
            final NodeList children = element.getChildNodes();
            for (int length = children.getLength(), loop = 0; loop < length; ++loop) {
                final Node currentNode = children.item(loop);
                if (currentNode.getNodeType() == 1) {
