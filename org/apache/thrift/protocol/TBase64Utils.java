...
class TBase64Utils
{
    ...
    static final void encode(final byte[] src, final int srcOff, final int len, final byte[] dst, final int dstOff) {
        ...
        if (len == 3) {
            dst[dstOff + 1] = (byte)"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt((src[srcOff] << 4 & 0x30) | (src[srcOff + 1] >> 4 & 0xF));
            dst[dstOff + 2] = (byte)"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt((src[srcOff + 1] << 2 & 0x3C) | (src[srcOff + 2] >> 6 & 0x3));
            dst[dstOff + 3] = (byte)"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt(src[srcOff + 2] & 0x3F);
        }
        else if (len == 2) {
            dst[dstOff + 1] = (byte)"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt((src[srcOff] << 4 & 0x30) | (src[srcOff + 1] >> 4 & 0xF));
            dst[dstOff + 2] = (byte)"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt(src[srcOff + 1] << 2 & 0x3C);
        }
        else {
            dst[dstOff + 1] = (byte)"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt(src[srcOff] << 4 & 0x30);
        }
    }
