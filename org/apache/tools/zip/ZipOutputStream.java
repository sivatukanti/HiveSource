...
public class ZipOutputStream extends FilterOutputStream
{
    ...
    public ZipOutputStream(final OutputStream out) {
        ...
        this.cdOffset = 0L;
        this.cdLength = 0L;
        this.offsets = new HashMap<ZipEntry, Long>();
        this.encoding = null;
        this.zipEncoding = ZipEncodingHelper.getZipEncoding(ZipOutputStream.DEFAULT_ENCODING);
    ...
    public void closeEntry() throws IOException {
        ...
        if (!this.entry.hasWritten) {
            this.write(ZipOutputStream.EMPTY, 0, 0);
        }
        this.flushDeflater();
    ...
    private void setDefaults(final ZipEntry entry) {
    ...
    private boolean shouldAddZip64Extra(final ZipEntry entry, final Zip64Mode mode) {
    ...
    public boolean canWriteEntryData(final ZipEntry ae) {
    ...
    protected void writeLocalFileHeader(final ZipEntry ze) throws IOException {
        ...
        this.writeOut(ZipOutputStream.LFH_SIG);
        this.written += 4L;
        ...
        if (zipMethod == 8 || this.raf != null) {
            this.writeOut(ZipOutputStream.LZERO);
            if (this.hasZip64Extra(this.entry.entry)) {
                this.writeOut(ZipLong.ZIP64_MAGIC.getBytes());
                this.writeOut(ZipLong.ZIP64_MAGIC.getBytes());
            }
    ...
    private void addUnicodeExtraFields(final ZipEntry ze, final boolean encodable, final ByteBuffer name) throws IOException {
    ...
    protected void writeDataDescriptor(final ZipEntry ze) throws IOException {
        if (ze.getMethod() != 8 || this.raf != null) {
            return;
        }
        this.writeOut(ZipOutputStream.DD_SIG);
        this.writeOut(ZipLong.getBytes(ze.getCrc()));
        int sizeFieldSize = 4;
        if (!this.hasZip64Extra(ze)) {
    ...
    protected void writeCentralFileHeader(final ZipEntry ze) throws IOException {
        this.writeOut(ZipOutputStream.CFH_SIG);
        this.written += 4L;
        ...
        this.written += 2L;
        this.writeOut(ZipOutputStream.ZERO);
        this.written += 2L;
    ...
    private void handleZip64Extra(final ZipEntry ze, final long lfhOffset, final boolean needsZip64Extra) {
        if (needsZip64Extra) {
    ...
    protected void writeCentralDirectoryEnd() throws IOException {
        this.writeOut(ZipOutputStream.EOCD_SIG);
        this.writeOut(ZipOutputStream.ZERO);
        this.writeOut(ZipOutputStream.ZERO);
    ...
    @Deprecated
    protected static ZipLong toDosTime(final Date time) {
        return ZipUtil.toDosTime(time);
    }
    
    @Deprecated
    protected static byte[] toDosTime(final long t) {
        return ZipUtil.toDosTime(t);
    }
    
    protected byte[] getBytes(final String name) throws ZipException {
        try {
            final ByteBuffer b = ZipEncodingHelper.getZipEncoding(this.encoding).encode(name);
            final byte[] result = new byte[b.limit()];
    ...
    protected void writeZip64CentralDirectory() throws IOException {
        ...
        this.writeOut(ZipOutputStream.ZIP64_EOCD_SIG);
        ...
        this.writeOut(ZipOutputStream.ZIP64_EOCD_LOC_SIG);
        this.writeOut(ZipOutputStream.LZERO);
        this.writeOut(ZipEightByteInteger.getBytes(offset));
        this.writeOut(ZipOutputStream.ONE);
    }
    ...
    @Deprecated
    protected static long adjustToLong(final int i) {
    ...
    private Zip64ExtendedInformationExtraField getZip64Extra(final ZipEntry ze) {
        if (this.entry != null) {
    ...
    private boolean hasZip64Extra(final ZipEntry ze) {
        return ze.getExtraField(Zip64ExtendedInformationExtraField.HEADER_ID) != null;
    }
    
    private Zip64Mode getEffectiveZip64Mode(final ZipEntry ze) {
    ...
    private ZipEncoding getEntryEncoding(final ZipEntry ze) {
        final boolean encodable = this.zipEncoding.canEncode(ze.getName());
        return (!encodable && this.fallbackToUTF8) ? ZipEncodingHelper.UTF8_ZIP_ENCODING : this.zipEncoding;
    }
    
    private ByteBuffer getName(final ZipEntry ze) throws IOException {
    ...
    private static final class CurrentEntry
    {
        ...
        private CurrentEntry(final ZipEntry entry) {
