...
public class ZipFile
{
    ...
    public ZipFile(final File f, final String encoding, final boolean useUnicodeExtraFields) throws IOException {
        this.entries = new LinkedHashMap<ZipEntry, OffsetEntry>(509);
        this.nameMap = new HashMap<String, ZipEntry>(509);
    ...
    public Enumeration<ZipEntry> getEntriesInPhysicalOrder() {
        final ZipEntry[] allEntries = this.entries.keySet().toArray(new ZipEntry[0]);
    ...
    private void resolveLocalFileHeaderData(final Map<ZipEntry, NameAndComment> entriesWithoutUTF8Flag) throws IOException {
        final Map<ZipEntry, OffsetEntry> origMap = new LinkedHashMap<ZipEntry, OffsetEntry>(this.entries);
        this.entries.clear();
        for (final Map.Entry<ZipEntry, OffsetEntry> ent : origMap.entrySet()) {
            final ZipEntry ze = ent.getKey();
