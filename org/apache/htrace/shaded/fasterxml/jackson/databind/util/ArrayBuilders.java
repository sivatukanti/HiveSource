...
public final class ArrayBuilders
{
    ...
    public static <T> ArrayList<T> arrayToList(final T[] elements) {
    ...
    public static <T> HashSet<T> setAndArray(final Set<T> set, final T[] elements) {
        final HashSet<T> result = new HashSet<T>();
        if (set != null) {
            result.addAll((Collection<?>)set);
        }
        if (elements != null) {
            for (final T value : elements) {
                result.add(value);
            }
        }
        return result;
