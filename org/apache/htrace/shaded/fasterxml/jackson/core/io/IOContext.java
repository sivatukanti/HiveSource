...
public class IOContext
{
    ...
    protected void _verifyAlloc(final Object buffer) {
        if (buffer != null) {
            throw new IllegalStateException("Trying to call same allocXxx() method second time");
        }
    }
    
    protected void _verifyRelease(final byte[] toRelease, final byte[] src) {
        if (toRelease != src && toRelease.length <= src.length) {
            throw this.wrongBuf();
        }
    }
    
    protected void _verifyRelease(final char[] toRelease, final char[] src) {
        if (toRelease != src && toRelease.length <= src.length) {
            throw this.wrongBuf();
        }
    }
