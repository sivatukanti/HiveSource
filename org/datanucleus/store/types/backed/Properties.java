...
public class Properties extends org.datanucleus.store.types.simple.Properties implements BackedSCO
{
    ...
    public Properties(final ObjectProvider op, final AbstractMemberMetaData mmd) {
        super(op, mmd);
        ...
        if (!SCOUtils.mapHasSerialisedKeysAndValues(mmd) && mmd.getPersistenceModifier() == FieldPersistenceModifier.PERSISTENT) {
            ...
            this.backingStore = (MapStore)((BackedSCOStoreManager)ec.getStoreManager()).getBackingStoreForField(clr, mmd, Map.class);
        }
    ...
    public void initialise(final Object o, final boolean forInsert, final boolean forUpdate) {
        ...
        if (m != null) {
            ...
            if (forInsert) {
                if (NucleusLogger.PERSISTENCE.isDebugEnabled()) {
                    NucleusLogger.PERSISTENCE.debug(Properties.LOCALISER.msg("023007", this.ownerOP.getObjectAsPrintable(), this.ownerMmd.getName(), "" + m.size()));
                }
                this.makeDirty();
                if (this.useCache) {
                    this.loadFromStore();
                ...
                if (NucleusLogger.PERSISTENCE.isDebugEnabled()) {
                    NucleusLogger.PERSISTENCE.debug(Properties.LOCALISER.msg("023008", this.ownerOP.getObjectAsPrintable(), this.ownerMmd.getName(), "" + m.size()));
                }
                this.delegate.clear();
                if (this.backingStore != null) {
                    if (SCOUtils.useQueuedUpdate(this.queued, this.ownerOP)) {
    ...
    protected void loadFromStore() {
        if (this.backingStore != null && !this.isCacheLoaded) {
            if (NucleusLogger.PERSISTENCE.isDebugEnabled()) {
                NucleusLogger.PERSISTENCE.debug(Properties.LOCALISER.msg("023006", this.ownerOP.getObjectAsPrintable(), this.ownerMmd.getName()));
            }
            this.delegate.clear();
    ...
    public String getProperty(final String key) {
        if (this.useCache) {
            ...
            final Object val = this.backingStore.get(this.ownerOP, key);
            final String strVal = (val instanceof String) ? ((String)val) : null;
