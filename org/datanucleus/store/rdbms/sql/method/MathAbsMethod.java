...
public class MathAbsMethod extends AbstractSQLMethod
{
    ...
    public SQLExpression getExpression(final SQLExpression ignore, final List args) {
        ...
        if (!(expr instanceof SQLLiteral)) {
            return this.exprFactory.invokeMethod(this.stmt, null, "abs", null, args);
        }
        if (expr instanceof ByteLiteral) {
            final int originalValue = ((BigInteger)((ByteLiteral)expr).getValue()).intValue();
            final BigInteger absValue = new BigInteger(String.valueOf(Math.abs(originalValue)));
        ...
        if (expr instanceof IntegerLiteral) {
            ...
            final Integer absValue2 = Math.abs(originalValue);
            return new IntegerLiteral(this.stmt, expr.getJavaTypeMapping(), absValue2, null);
        }
        if (expr instanceof FloatingPointLiteral) {
            ...
            final Double absValue3 = new Double(Math.abs(originalValue2));
