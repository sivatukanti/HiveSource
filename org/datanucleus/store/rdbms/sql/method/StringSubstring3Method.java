...
public class StringSubstring3Method extends AbstractSQLMethod
{
    ...
    public SQLExpression getExpression(final SQLExpression expr, final List args) {
        if (args == null || args.size() == 0 || args.size() > 2) {
            throw new NucleusException(StringSubstring3Method.LOCALISER.msg("060003", "substring", "StringExpression", 0, "NumericExpression/IntegerLiteral/ParameterLiteral"));
        }
        if (args.size() == 1) {
            ...
            if (!(startExpr instanceof NumericExpression)) {
                throw new NucleusException(StringSubstring3Method.LOCALISER.msg("060003", "substring", "StringExpression", 0, "NumericExpression"));
            }
            final SQLExpression endExpr = args.get(1);
            if (!(endExpr instanceof NumericExpression)) {
                throw new NucleusException(StringSubstring3Method.LOCALISER.msg("060003", "substring", "StringExpression", 1, "NumericExpression"));
            }
            ...
            return new StringExpression(this.stmt, this.getMappingForClass(String.class), "SUBSTR", funcArgs2);
        }
    }
}
