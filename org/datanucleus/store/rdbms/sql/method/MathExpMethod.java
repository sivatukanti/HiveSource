...
public class MathExpMethod extends AbstractSQLMethod
{
    ...
    public SQLExpression getExpression(final SQLExpression ignore, final List args) {
        ...
        if (!(expr instanceof SQLLiteral)) {
            return this.exprFactory.invokeMethod(this.stmt, null, "exp", null, args);
        }
        if (expr instanceof ByteLiteral) {
            final int originalValue = ((BigInteger)((ByteLiteral)expr).getValue()).intValue();
            final BigInteger absValue = new BigInteger(String.valueOf(Math.exp(originalValue)));
        ...
        if (expr instanceof IntegerLiteral) {
            ...
            final Double absValue2 = new Double(Math.exp(originalValue));
        ...
        if (expr instanceof FloatingPointLiteral) {
            ...
            final Double absValue3 = new Double(Math.exp(originalValue2));
