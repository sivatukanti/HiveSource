...
public class JDOPropertyGetterAdapter extends MethodVisitor
{
    ...
    public JDOPropertyGetterAdapter(final MethodVisitor mv, final ClassEnhancer enhancer, final String methodName, final String methodDesc, final AbstractMemberMetaData mmd, final ClassVisitor cv) {
        super(262144, mv);
        this.visitor = null;
    ...
    public void visitEnd() {
        ...
        if (DataNucleusEnhancer.LOGGER.isDebugEnabled()) {
            final String msg = ClassMethod.getMethodAdditionMessage(this.enhancer.getNamer().getGetMethodPrefixMethodName() + this.mmd.getName(), this.mmd.getType(), null, null);
            DataNucleusEnhancer.LOGGER.debug(JDOPropertyGetterAdapter.LOCALISER.msg("Enhancer.AddMethod", msg));
        }
    ...
    public static void generateGetXXXMethod(final MethodVisitor mv, final AbstractMemberMetaData mmd, final String asmClassName, final String asmClassDesc, final boolean detachListener, final boolean includeFrames, final EnhancementNamer namer) {
        final String[] argNames = { "objPC" };
        final String fieldTypeDesc = Type.getDescriptor(mmd.getType());
        mv.visitCode();
        final AbstractClassMetaData cmd = mmd.getAbstractClassMetaData();
        if ((mmd.getPersistenceFlags() & 0x2) == 0x2) {
            ...
            final String methodName = "get" + EnhanceUtils.getTypeNameForJDOMethod(mmd.getType()) + "Field";
            ...
            if (cmd.isDetachable()) {
                ...
                if (detachListener) {
                    ...
                    mv.visitInsn(89);
                    mv.visitLdcInsn(JDOPropertyGetterAdapter.LOCALISER.msg("Enhancer.DetachedPropertyAccess", mmd.getName()));
        ...
        else if ((mmd.getPersistenceFlags() & 0x1) == 0x1) {
            final Label startLabel = new Label();
            mv.visitLabel(startLabel);
            mv.visitVarInsn(25, 0);
            ...
            mv.visitLabel(endLabel3);
            mv.visitLocalVariable(argNames[0], asmClassDesc, null, startLabel, endLabel3, 0);
            mv.visitMaxs(1, 1);
        }
        mv.visitEnd();
    }
