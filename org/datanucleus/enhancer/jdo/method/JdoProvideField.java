...
{
    public static JdoProvideField getInstance(final ClassEnhancer enhancer) {
        return new JdoProvideField(enhancer, enhancer.getNamer().getProvideFieldMethodName(), 1, null, new Class[] { Integer.TYPE }, new String[] { "index" });
    }
    
    public JdoProvideField(final ClassEnhancer enhancer, final String name, final int access, final Object returnType, final Object[] argTypes, final String[] argNames) {
        ...
        if (pcSuperclassName != null) {
            if (fields.length > 0) {
                ...
                for (int j = 0; j < fields.length; ++j) {
                    ...
                    this.visitor.visitMethodInsn(185, this.getNamer().getStateManagerAsmClassName(), "provided" + EnhanceUtils.getTypeNameForJDOMethod(fields[j].getType()) + "Field", "(" + this.getNamer().getPersistableDescriptor() + "I" + EnhanceUtils.getTypeDescriptorForJDOMethod(fields[j].getType()) + ")V");
            ...
            this.visitor.visitLocalVariable("this", this.getClassEnhancer().getClassDescriptor(), null, startLabel, l2, 0);
            this.visitor.visitLocalVariable(this.argNames[0], "I", null, startLabel, l2, 1);
