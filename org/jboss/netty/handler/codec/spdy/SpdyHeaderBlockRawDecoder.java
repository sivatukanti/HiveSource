...
public class SpdyHeaderBlockRawDecoder extends SpdyHeaderBlockDecoder
{
    ...
    private int readLengthField(final ChannelBuffer buffer) {
    ...
    void decode(final ChannelBuffer headerBlock, final SpdyHeadersFrame frame) throws Exception {
        ...
        if (this.cumulation == null) {
            ...
            if (headerBlock.readable()) {
                (this.cumulation = ChannelBuffers.dynamicBuffer(headerBlock.readableBytes())).writeBytes(headerBlock);
            }
        }
        else {
    ...
    protected void decodeHeaderBlock(final ChannelBuffer headerBlock, final SpdyHeadersFrame frame) throws Exception {
        while (headerBlock.readable()) {
            switch (this.state) {
                case READ_NUM_HEADERS: {
                    ...
                    this.numHeaders = this.readLengthField(headerBlock);
                ...
                case READ_NAME_LENGTH: {
                    ...
                    this.length = this.readLengthField(headerBlock);
                    if (this.length <= 0) {
                        this.state = State.ERROR;
                        frame.setInvalid();
                        continue;
