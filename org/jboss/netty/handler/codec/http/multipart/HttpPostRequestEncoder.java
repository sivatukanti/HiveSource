...
public class HttpPostRequestEncoder implements ChunkedInput
{
    ...
    public HttpPostRequestEncoder(final HttpDataFactory factory, final HttpRequest request, final boolean multipart, final Charset charset) throws ErrorDataEncoderException {
    ...
    public void addBodyHttpData(final InterfaceHttpData data) throws ErrorDataEncoderException {
        ...
        if (data instanceof Attribute) {
            ...
            if (contentTransferEncoding != null && contentTransferEncoding.equals(HttpPostBodyUtil.TransferEncodingMechanism.BINARY.value())) {
                ...
                internal2.addValue("; charset=" + fileUpload.getCharset().name() + "\r\n\r\n");
            }
            else {
                internal2.addValue("\r\n\r\n");
            }
    ...
    public HttpRequest finalizeRequest() throws ErrorDataEncoderException {
        ...
        if (!this.headerFinalized) {
            ...
            if (realSize > 8096L || this.isMultipart) {
                ...
                final HttpChunk chunk = this.nextChunk();
    ...
    private ChannelBuffer fillChannelBuffer() {
        ...
        if (length > 8096) {
            final ChannelBuffer slice = this.currentBuffer.slice(this.currentBuffer.readerIndex(), 8096);
            this.currentBuffer.skipBytes(8096);
            return slice;
        }
        final ChannelBuffer slice = this.currentBuffer;
        this.currentBuffer = null;
        return slice;
    }
    
    private HttpChunk encodeNextChunkMultipart(final int sizeleft) throws ErrorDataEncoderException {
    ...
    private HttpChunk encodeNextChunkUrlEncoded(final int sizeleft) throws ErrorDataEncoderException {
        if (this.currentData == null) {
            return null;
        }
        int size = sizeleft;
        if (this.isKey) {
            ...
            ChannelBuffer buffer = ChannelBuffers.wrappedBuffer(key.getBytes());
            this.isKey = false;
            if (this.currentBuffer == null) {
    ...
    public HttpChunk nextChunk() throws ErrorDataEncoderException {
        ...
        if (size <= 0) {
            final ChannelBuffer buffer = this.fillChannelBuffer();
            return new DefaultHttpChunk(buffer);
        }
        if (this.currentData != null) {
            if (this.isMultipart) {
                final HttpChunk chunk = this.encodeNextChunkMultipart(size);
                if (chunk != null) {
                    return chunk;
                }
            }
            else {
                final HttpChunk chunk = this.encodeNextChunkUrlEncoded(size);
                if (chunk != null) {
                    return chunk;
                }
            }
        ...
        if (!this.iterator.hasNext()) {
            ...
            final ChannelBuffer buffer = this.currentBuffer;
            this.currentBuffer = null;
            return new DefaultHttpChunk(buffer);
        }
