...
public class WebSocketServerHandshaker13 extends WebSocketServerHandshaker
{
    ...
    public WebSocketServerHandshaker13(final String webSocketURL, final String subprotocols, final boolean allowExtensions) {
        this(webSocketURL, subprotocols, allowExtensions, Long.MAX_VALUE);
    }
    
    public WebSocketServerHandshaker13(final String webSocketURL, final String subprotocols, final boolean allowExtensions, final long maxFramePayloadLength) {
    ...
    public ChannelFuture handshake(final Channel channel, final HttpRequest req) {
        if (WebSocketServerHandshaker13.logger.isDebugEnabled()) {
            WebSocketServerHandshaker13.logger.debug(String.format("Channel %s WS Version 13 server handshake", channel.getId()));
        }
        ...
        if (WebSocketServerHandshaker13.logger.isDebugEnabled()) {
            WebSocketServerHandshaker13.logger.debug(String.format("WS Version 13 Server Handshake key: %s. Response: %s.", key, accept));
        }
    ...
    static {
        logger = InternalLoggerFactory.getInstance(WebSocketServerHandshaker13.class);
    }
}
