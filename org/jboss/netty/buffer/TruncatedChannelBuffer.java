...
public class TruncatedChannelBuffer extends AbstractChannelBuffer implements WrappedChannelBuffer
{
    ...
    public TruncatedChannelBuffer(final ChannelBuffer buffer, final int length) {
    ...
    public ChannelBuffer duplicate() {
        final ChannelBuffer duplicate = new TruncatedChannelBuffer(this.buffer, this.length);
    ...
    private void checkIndex(final int index) {
        if (index < 0 || index >= this.capacity()) {
            throw new IndexOutOfBoundsException("Invalid index of " + index + ", maximum is " + this.capacity());
        }
    }
    ...
    private void checkIndex(final int index, final int length) {
        ...
        if (index + length > this.capacity()) {
            throw new IndexOutOfBoundsException("Invalid index of " + (index + length) + ", maximum is " + this.capacity());
        }
    }
}
