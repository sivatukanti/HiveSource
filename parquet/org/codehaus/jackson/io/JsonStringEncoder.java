...
import parquet.org.codehaus.jackson.util.TextBuffer;
...
public final class JsonStringEncoder
{
        ...
        while (inPtr < inputLen) {
            while (true) {
                ...
                if (c < escCodeCount && escCodes[c] != 0) {
                    final int escCode = escCodes[input.charAt(inPtr++)];
                    final int length = this._appendSingleEscape(escCode, this._quoteBuffer);
    ...
    private int _appendSingleEscape(final int escCode, final char[] quoteBuffer) {
        if (escCode < 0) {
