...
public class TSaslServerTransport extends TSaslTransport
{
    ...
    protected void handleSaslStartMessage() throws TTransportException, SaslException {
        ...
        if (message.status != NegotiationStatus.START) {
            this.sendAndThrowMessage(NegotiationStatus.ERROR, "Expecting START status, received " + message.status);
        }
        ...
        if (serverDefinition == null) {
            this.sendAndThrowMessage(NegotiationStatus.BAD, "Unsupported mechanism type " + mechanismName);
        }
    ...
    public static class Factory extends TTransportFactory
    {
        ...
        public TTransport getTransport(final TTransport base) {
            TSaslServerTransport ret = Factory.transportMap.get(base);
            if (ret == null) {
                TSaslServerTransport.LOGGER.debug("transport map does not contain key", base);
                ret = new TSaslServerTransport(this.serverDefinitionMap, base, null);
                try {
                    ret.open();
                }
        ...
        static {
            Factory.transportMap = Collections.synchronizedMap(new WeakHashMap<TTransport, TSaslServerTransport>());
        }
    }
}
