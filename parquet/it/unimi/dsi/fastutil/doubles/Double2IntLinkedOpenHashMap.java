...
public class Double2IntLinkedOpenHashMap extends AbstractDouble2IntSortedMap implements Serializable, Cloneable, Hash
{
    ...
    protected int size;
    protected transient volatile Double2IntSortedMap.FastSortedEntrySet entries;
    ...
    public int put(final double k, final int v) {
        int pos;
        for (pos = ((int)HashCommon.murmurHash3(Double.doubleToRawLongBits(k) ^ (long)this.mask) & this.mask); this.used[pos]; pos = (pos + 1 & this.mask)) {
            if (this.key[pos] == k) {
    ...
    protected final int shiftKeys(int pos) {
        ...
        while (true) {
            ...
            for (pos = ((last = pos) + 1 & this.mask); this.used[pos]; pos = (pos + 1 & this.mask)) {
                slot = ((int)HashCommon.murmurHash3(Double.doubleToRawLongBits(this.key[pos]) ^ (long)this.mask) & this.mask);
                if (last <= pos) {
                    if (last >= slot) {
                        break;
                    }
    ...
    public int remove(final double k) {
        for (int pos = (int)HashCommon.murmurHash3(Double.doubleToRawLongBits(k) ^ (long)this.mask) & this.mask; this.used[pos]; pos = (pos + 1 & this.mask)) {
            if (this.key[pos] == k) {
                --this.size;
    ...
    public int getAndMoveToFirst(final double k) {
        ...
        for (int mask = this.mask, pos = (int)HashCommon.murmurHash3(Double.doubleToRawLongBits(k) ^ (long)mask) & mask; used[pos]; pos = (pos + 1 & mask)) {
            if (k == key[pos]) {
    ...
    public int putAndMoveToFirst(final double k, final int v) {
        ...
        int pos;
        int oldValue;
        for (used = this.used, mask = this.mask, pos = ((int)HashCommon.murmurHash3(Double.doubleToRawLongBits(k) ^ (long)mask) & mask); used[pos]; pos = (pos + 1 & mask)) {
            if (k == key[pos]) {
    ...
    protected void rehash(final int newN) {
        ...
        int j = this.size;
        while (j-- != 0) {
            final double k = key[i];
            int pos;
            for (pos = ((int)HashCommon.murmurHash3(Double.doubleToRawLongBits(k) ^ (long)mask) & mask); newUsed[pos]; pos = (pos + 1 & mask)) {}
            newUsed[pos] = true;
            newKey[pos] = k;
    ...
    private void readObject(final ObjectInputStream s) throws IOException, ClassNotFoundException {
        ...
        this.maxFill = HashCommon.maxFill(this.n, this.f);
        this.mask = this.n - 1;
        final double[] key2 = new double[this.n];
        this.key = key2;
        ...
        while (i-- != 0) {
            ...
            for (pos = ((int)HashCommon.murmurHash3(Double.doubleToRawLongBits(k) ^ (long)this.mask) & this.mask); used[pos]; pos = (pos + 1 & this.mask)) {}
            used[pos] = true;
            key[pos] = k;
            value[pos] = v;
            if (this.first != -1) {
    ...
    private final class MapEntry implements Double2IntMap.Entry, Map.Entry<Double, Integer>
    {
        ...
        public int setValue(final int v) {
            final int oldValue = Double2IntLinkedOpenHashMap.this.value[this.index];
        ...
        public boolean equals(final Object o) {
            ...
            return Double2IntLinkedOpenHashMap.this.key[this.index] == e.getKey() && Double2IntLinkedOpenHashMap.this.value[this.index] == e.getValue();
        }
    ...
    private class MapIterator
    {
        ...
        private MapIterator(final double from) {
            ...
            for (int pos = (int)HashCommon.murmurHash3(Double.doubleToRawLongBits(from) ^ (long)Double2IntLinkedOpenHashMap.this.mask) & Double2IntLinkedOpenHashMap.this.mask; Double2IntLinkedOpenHashMap.this.used[pos]; pos = (pos + 1 & Double2IntLinkedOpenHashMap.this.mask)) {
        ...
        public void remove() {
            ...
            final Double2IntLinkedOpenHashMap this$0 = Double2IntLinkedOpenHashMap.this;
            --this$0.size;
            if (this.prev == -1) {
                Double2IntLinkedOpenHashMap.this.first = this.next;
            }
            else {
                final long[] link = Double2IntLinkedOpenHashMap.this.link;
                final int prev = this.prev;
                link[prev] ^= ((Double2IntLinkedOpenHashMap.this.link[this.prev] ^ ((long)this.next & 0xFFFFFFFFL)) & 0xFFFFFFFFL);
            }
            if (this.next == -1) {
                Double2IntLinkedOpenHashMap.this.last = this.prev;
            }
            else {
                final long[] link2 = Double2IntLinkedOpenHashMap.this.link;
                final int next = this.next;
                link2[next] ^= ((Double2IntLinkedOpenHashMap.this.link[this.next] ^ ((long)this.prev & 0xFFFFFFFFL) << 32) & 0xFFFFFFFF00000000L);
            }
            int pos = this.curr;
            int last;
            while (true) {
                for (pos = ((last = pos) + 1 & Double2IntLinkedOpenHashMap.this.mask); Double2IntLinkedOpenHashMap.this.used[pos]; pos = (pos + 1 & Double2IntLinkedOpenHashMap.this.mask)) {
                    final int slot = (int)HashCommon.murmurHash3(Double.doubleToRawLongBits(Double2IntLinkedOpenHashMap.this.key[pos]) ^ (long)Double2IntLinkedOpenHashMap.this.mask) & Double2IntLinkedOpenHashMap.this.mask;
                    if (last <= pos) {
                        if (last >= slot) {
                            break;
                        }
                ...
                if (!Double2IntLinkedOpenHashMap.this.used[pos]) {
                    break;
                }
    ...
    private class EntryIterator extends MapIterator implements ObjectListIterator<Double2IntMap.Entry>
    {
        ...
        @Override
        public void set(final Double2IntMap.Entry ok) {
            throw new UnsupportedOperationException();
        }
        
        @Override
        public void add(final Double2IntMap.Entry ok) {
            throw new UnsupportedOperationException();
        }
    }
    ...
    private class FastEntryIterator extends MapIterator implements ObjectListIterator<Double2IntMap.Entry>
    {
        ...
        public FastEntryIterator() {
            this.entry = new BasicEntry(0.0, 0);
        }
        ...
        public BasicEntry next() {
            ...
            this.entry.key = Double2IntLinkedOpenHashMap.this.key[e];
            this.entry.value = Double2IntLinkedOpenHashMap.this.value[e];
            return this.entry;
        }
    ...
    private final class MapEntrySet extends AbstractObjectSortedSet<Double2IntMap.Entry> implements Double2IntSortedMap.FastSortedEntrySet
    {
        ...
        @Override
        public ObjectSortedSet<Double2IntMap.Entry> subSet(final Double2IntMap.Entry fromElement, final Double2IntMap.Entry toElement) {
            throw new UnsupportedOperationException();
        }
        
        @Override
        public ObjectSortedSet<Double2IntMap.Entry> headSet(final Double2IntMap.Entry toElement) {
            throw new UnsupportedOperationException();
        }
        
        @Override
        public ObjectSortedSet<Double2IntMap.Entry> tailSet(final Double2IntMap.Entry fromElement) {
            throw new UnsupportedOperationException();
        }
        ...
        public boolean contains(final Object o) {
            ...
            for (int pos = (int)HashCommon.murmurHash3(Double.doubleToRawLongBits(k) ^ (long)Double2IntLinkedOpenHashMap.this.mask) & Double2IntLinkedOpenHashMap.this.mask; Double2IntLinkedOpenHashMap.this.used[pos]; pos = (pos + 1 & Double2IntLinkedOpenHashMap.this.mask)) {
                if (Double2IntLinkedOpenHashMap.this.key[pos] == k) {
                    return Double2IntLinkedOpenHashMap.this.value[pos] == e.getValue();
                }
            }
            return false;
        }
        ...
        public ObjectBidirectionalIterator<Double2IntMap.Entry> iterator(final Double2IntMap.Entry from) {
            return new EntryIterator(((Map.Entry<Double, V>)from).getKey());
        }
        ...
        public ObjectBidirectionalIterator<Double2IntMap.Entry> fastIterator(final Double2IntMap.Entry from) {
            return new FastEntryIterator(((Map.Entry<Double, V>)from).getKey());
        }
    }
    ...
    private final class KeySet extends AbstractDoubleSortedSet
    {
        ...
        public boolean remove(final double k) {
            final int oldSize = Double2IntLinkedOpenHashMap.this.size;
