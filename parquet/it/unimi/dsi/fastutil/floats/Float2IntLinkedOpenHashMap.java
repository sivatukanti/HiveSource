...
public class Float2IntLinkedOpenHashMap extends AbstractFloat2IntSortedMap implements Serializable, Cloneable, Hash
{
    ...
    protected int size;
    protected transient volatile Float2IntSortedMap.FastSortedEntrySet entries;
    ...
    public int put(final float k, final int v) {
        int pos;
        for (pos = (HashCommon.murmurHash3(HashCommon.float2int(k) ^ this.mask) & this.mask); this.used[pos]; pos = (pos + 1 & this.mask)) {
            if (this.key[pos] == k) {
    ...
    protected final int shiftKeys(int pos) {
        ...
        while (true) {
            ...
            for (pos = ((last = pos) + 1 & this.mask); this.used[pos]; pos = (pos + 1 & this.mask)) {
                slot = (HashCommon.murmurHash3(HashCommon.float2int(this.key[pos]) ^ this.mask) & this.mask);
                if (last <= pos) {
                    if (last >= slot) {
                        break;
                    }
    ...
    public int remove(final float k) {
        for (int pos = HashCommon.murmurHash3(HashCommon.float2int(k) ^ this.mask) & this.mask; this.used[pos]; pos = (pos + 1 & this.mask)) {
            if (this.key[pos] == k) {
                --this.size;
    ...
    public int getAndMoveToFirst(final float k) {
        ...
        for (int mask = this.mask, pos = HashCommon.murmurHash3(HashCommon.float2int(k) ^ mask) & mask; used[pos]; pos = (pos + 1 & mask)) {
            if (k == key[pos]) {
    ...
    public int putAndMoveToFirst(final float k, final int v) {
        ...
        int pos;
        int oldValue;
        for (used = this.used, mask = this.mask, pos = (HashCommon.murmurHash3(HashCommon.float2int(k) ^ mask) & mask); used[pos]; pos = (pos + 1 & mask)) {
            if (k == key[pos]) {
    ...
    protected void rehash(final int newN) {
        ...
        while (j-- != 0) {
            final float k = key[i];
            int pos;
            for (pos = (HashCommon.murmurHash3(HashCommon.float2int(k) ^ mask) & mask); newUsed[pos]; pos = (pos + 1 & mask)) {}
            newUsed[pos] = true;
            newKey[pos] = k;
    ...
    private void readObject(final ObjectInputStream s) throws IOException, ClassNotFoundException {
        ...
        this.maxFill = HashCommon.maxFill(this.n, this.f);
        this.mask = this.n - 1;
        final float[] key2 = new float[this.n];
        this.key = key2;
        ...
        while (i-- != 0) {
            ...
            for (pos = (HashCommon.murmurHash3(HashCommon.float2int(k) ^ this.mask) & this.mask); used[pos]; pos = (pos + 1 & this.mask)) {}
            used[pos] = true;
            key[pos] = k;
            value[pos] = v;
            if (this.first != -1) {
    ...
    private final class MapEntry implements Float2IntMap.Entry, Map.Entry<Float, Integer>
    {
        ...
        public boolean equals(final Object o) {
            ...
            return Float2IntLinkedOpenHashMap.this.key[this.index] == e.getKey() && Float2IntLinkedOpenHashMap.this.value[this.index] == e.getValue();
        }
    ...
    private class MapIterator
    {
        ...
        private MapIterator(final float from) {
            ...
            for (int pos = HashCommon.murmurHash3(HashCommon.float2int(from) ^ Float2IntLinkedOpenHashMap.this.mask) & Float2IntLinkedOpenHashMap.this.mask; Float2IntLinkedOpenHashMap.this.used[pos]; pos = (pos + 1 & Float2IntLinkedOpenHashMap.this.mask)) {
        ...
        public void remove() {
            ...
            int last;
            while (true) {
                for (pos = ((last = pos) + 1 & Float2IntLinkedOpenHashMap.this.mask); Float2IntLinkedOpenHashMap.this.used[pos]; pos = (pos + 1 & Float2IntLinkedOpenHashMap.this.mask)) {
                    final int slot = HashCommon.murmurHash3(HashCommon.float2int(Float2IntLinkedOpenHashMap.this.key[pos]) ^ Float2IntLinkedOpenHashMap.this.mask) & Float2IntLinkedOpenHashMap.this.mask;
                    if (last <= pos) {
                        if (last >= slot) {
                            break;
                        }
                ...
                if (!Float2IntLinkedOpenHashMap.this.used[pos]) {
                    break;
                }
    ...
    private class FastEntryIterator extends MapIterator implements ObjectListIterator<Float2IntMap.Entry>
    {
        ...
        public FastEntryIterator() {
            this.entry = new BasicEntry(0.0f, 0);
        }
        ...
        public BasicEntry next() {
            ...
            this.entry.key = Float2IntLinkedOpenHashMap.this.key[e];
            this.entry.value = Float2IntLinkedOpenHashMap.this.value[e];
            return this.entry;
        }
    ...
    private final class MapEntrySet extends AbstractObjectSortedSet<Float2IntMap.Entry> implements Float2IntSortedMap.FastSortedEntrySet
    {
        ...
        public boolean contains(final Object o) {
            ...
            final Map.Entry<Float, Integer> e = (Map.Entry<Float, Integer>)o;
            final float k = e.getKey();
            for (int pos = HashCommon.murmurHash3(HashCommon.float2int(k) ^ Float2IntLinkedOpenHashMap.this.mask) & Float2IntLinkedOpenHashMap.this.mask; Float2IntLinkedOpenHashMap.this.used[pos]; pos = (pos + 1 & Float2IntLinkedOpenHashMap.this.mask)) {
                if (Float2IntLinkedOpenHashMap.this.key[pos] == k) {
                    return Float2IntLinkedOpenHashMap.this.value[pos] == e.getValue();
                }
            }
            return false;
        }
        ...
        public ObjectBidirectionalIterator<Float2IntMap.Entry> iterator(final Float2IntMap.Entry from) {
            return new EntryIterator(((Map.Entry<Float, V>)from).getKey());
        }
        ...
        public ObjectBidirectionalIterator<Float2IntMap.Entry> fastIterator(final Float2IntMap.Entry from) {
            return new FastEntryIterator(((Map.Entry<Float, V>)from).getKey());
        }
    }
