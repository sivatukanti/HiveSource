...
public class ObjectArrayList<K> extends AbstractObjectList<K> implements RandomAccess, Cloneable, Serializable
{
    ...
    private void grow(final int capacity) {
        if (this.wrapped) {
            ...
            final int newLength = (int)Math.min(Math.max(2L * this.a.length, capacity), 2147483639L);
    ...
    @Override
    public void clear() {
        ObjectArrays.fill(this.a, 0, this.size, (K)null);
        this.size = 0;
    }
    ...
    public void size(final int size) {
        ...
        if (size > this.size) {
            ObjectArrays.fill(this.a, this.size, size, (K)null);
        }
        else {
            ObjectArrays.fill(this.a, size, this.size, (K)null);
        }
        this.size = size;
    }
