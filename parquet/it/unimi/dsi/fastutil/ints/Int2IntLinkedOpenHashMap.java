...
public class Int2IntLinkedOpenHashMap extends AbstractInt2IntSortedMap implements Serializable, Cloneable, Hash
{
    ...
    public int put(final int k, final int v) {
        int pos;
        for (pos = (HashCommon.murmurHash3(k ^ this.mask) & this.mask); this.used[pos]; pos = (pos + 1 & this.mask)) {
            if (this.key[pos] == k) {
    ...
    protected final int shiftKeys(int pos) {
        ...
        while (true) {
            ...
            for (pos = ((last = pos) + 1 & this.mask); this.used[pos]; pos = (pos + 1 & this.mask)) {
                slot = (HashCommon.murmurHash3(this.key[pos] ^ this.mask) & this.mask);
                if (last <= pos) {
                    if (last >= slot) {
                        break;
                    }
    ...
    public int remove(final int k) {
        for (int pos = HashCommon.murmurHash3(k ^ this.mask) & this.mask; this.used[pos]; pos = (pos + 1 & this.mask)) {
            if (this.key[pos] == k) {
                --this.size;
    ...
    public int getAndMoveToFirst(final int k) {
        ...
        for (int mask = this.mask, pos = HashCommon.murmurHash3(k ^ mask) & mask; used[pos]; pos = (pos + 1 & mask)) {
            if (k == key[pos]) {
    ...
    public int putAndMoveToFirst(final int k, final int v) {
        ...
        int pos;
        int oldValue;
        for (used = this.used, mask = this.mask, pos = (HashCommon.murmurHash3(k ^ mask) & mask); used[pos]; pos = (pos + 1 & mask)) {
            if (k == key[pos]) {
    ...
    protected void rehash(final int newN) {
        ...
        while (j-- != 0) {
            final int k = key[i];
            int pos;
            for (pos = (HashCommon.murmurHash3(k ^ mask) & mask); newUsed[pos]; pos = (pos + 1 & mask)) {}
            newUsed[pos] = true;
            newKey[pos] = k;
    ...
    private void readObject(final ObjectInputStream s) throws IOException, ClassNotFoundException {
        ...
        while (i-- != 0) {
            ...
            for (pos = (HashCommon.murmurHash3(k ^ this.mask) & this.mask); used[pos]; pos = (pos + 1 & this.mask)) {}
            used[pos] = true;
            key[pos] = k;
            value[pos] = v;
            if (this.first != -1) {
    ...
    private class MapIterator
    {
        ...
        private MapIterator(final int from) {
            ...
            for (int pos = HashCommon.murmurHash3(from ^ Int2IntLinkedOpenHashMap.this.mask) & Int2IntLinkedOpenHashMap.this.mask; Int2IntLinkedOpenHashMap.this.used[pos]; pos = (pos + 1 & Int2IntLinkedOpenHashMap.this.mask)) {
        ...
        public void remove() {
            ...
            int last;
            while (true) {
                for (pos = ((last = pos) + 1 & Int2IntLinkedOpenHashMap.this.mask); Int2IntLinkedOpenHashMap.this.used[pos]; pos = (pos + 1 & Int2IntLinkedOpenHashMap.this.mask)) {
                    final int slot = HashCommon.murmurHash3(Int2IntLinkedOpenHashMap.this.key[pos] ^ Int2IntLinkedOpenHashMap.this.mask) & Int2IntLinkedOpenHashMap.this.mask;
                    if (last <= pos) {
                        if (last >= slot) {
                            break;
                        }
                ...
                if (!Int2IntLinkedOpenHashMap.this.used[pos]) {
                    break;
                }
    ...
    private class FastEntryIterator extends MapIterator implements ObjectListIterator<Int2IntMap.Entry>
    {
        ...
        public BasicEntry next() {
            ...
            this.entry.key = Int2IntLinkedOpenHashMap.this.key[e];
            this.entry.value = Int2IntLinkedOpenHashMap.this.value[e];
            return this.entry;
        }
    ...
    private final class MapEntrySet extends AbstractObjectSortedSet<Int2IntMap.Entry> implements Int2IntSortedMap.FastSortedEntrySet
    {
        ...
        public boolean contains(final Object o) {
            ...
            for (int pos = HashCommon.murmurHash3(k ^ Int2IntLinkedOpenHashMap.this.mask) & Int2IntLinkedOpenHashMap.this.mask; Int2IntLinkedOpenHashMap.this.used[pos]; pos = (pos + 1 & Int2IntLinkedOpenHashMap.this.mask)) {
                if (Int2IntLinkedOpenHashMap.this.key[pos] == k) {
                    return Int2IntLinkedOpenHashMap.this.value[pos] == e.getValue();
                }
            }
            return false;
        }
        ...
        public ObjectBidirectionalIterator<Int2IntMap.Entry> iterator(final Int2IntMap.Entry from) {
            return new EntryIterator(((Map.Entry<Integer, V>)from).getKey());
        }
        ...
        public ObjectBidirectionalIterator<Int2IntMap.Entry> fastIterator(final Int2IntMap.Entry from) {
            return new FastEntryIterator(((Map.Entry<Integer, V>)from).getKey());
        }
    }
